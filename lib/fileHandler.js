...
var client = upyun(Config.bucketname, Config.username, Config.password);
...
FileHandler.prototype = {
    ...
    prepareUpload: function() {
        ...
        this.each(function(file, done) {
            ...
            file.cdnName = md5(file.buffer.toString()) + path.extname(file.name);
            done();
        });
    },
    ...
    upload: function(callback) {
        ...
        this.each(function(file, done) {
            client.uploadFile(file.cdnName, file.buffer, function(err, status, data) {
                if (inter) {
                    clearInterval(inter);
                    inter = null;
                    console.log();
    ...
    _postOneFileUpload: function(file, done) {
        var that = this;
        var hostUrl = Config.url + '/' + file.cdnName;
    ...
    postUpload: function() {
        this.info('👍  Uploaded ' + this.strLength() + ' to Alipay cdn ' +
